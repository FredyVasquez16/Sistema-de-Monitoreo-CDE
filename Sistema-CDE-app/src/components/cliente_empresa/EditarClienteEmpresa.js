...
import { actualizarClienteEmpresa, guardarClienteEmpresa } from '../../actions/ClienteEmpresaAction';
import { useStateValue } from '../../Context/store';
import CircularProgress from '@material-ui/core/CircularProgress';
import { obtenerTiposClientesNivel, obtenerServiciosSolicitados, obtenerTiposClientesEstado, buscarAsesoresPorTermino, obtenerTiposOrganizacion, obtenerTiposEmpresa, obtenerTamanosEmpresa, obtenerClienteEmpresaPorId } from '../../actions/ClienteEmpresaAction';
...
const EditarClienteEmpresa = () => {
    ...
    // Estados para los valores seleccionados de los Autocomplete
    const [formState, setFormState] = useState({
    ...
    // Cargar datos del cliente/empresa al montar
    // Helper para normalizar fechas ISO a YYYY-MM-DD
    const normalizeDate = (dateString) => {
        if (!dateString) return '';
        // Si ya está en formato YYYY-MM-DD, retorna igual
        if (/^\d{4}-\d{2}-\d{2}$/.test(dateString)) return dateString;
        // Si es formato ISO, extrae solo la fecha
        return dateString.split('T')[0];
    };

    useEffect(() => {
        const cargarClienteEmpresa = async () => {
            try {
                const data = await obtenerClienteEmpresaPorId(id);
                console.log("Datos del cliente/empresa obtenidos:", data); // Depuración
                setFormState({
                    nombreCliente: data.nombre || '',
                    nivelCliente: data.tipoClienteNivelId || '',
                    contactoPrimarioId: data.contactoPrimarioId || null,
                    estadoCliente: data.tipoClienteEstadoId || '',
                    asesorPrincipalId: data.usuarioId || null,
                    nombrePropietarioId: data.nombrePropietarioId || null,
                    generoPropietario: data.generoPropietario || '',
                    razonSocial: data.razonSocial || '',
                    serviciosSolicitados: data.servicioSolicitadoId || '',
                    direccionFisica: data.direccionFisica || '',
                    municipio: data.municipio || '',
                    departamento: data.departamento || '',
                    numeroTelefono: data.telefono ? data.telefono.toString() : '',
                    paginaWeb: data.paginaWeb || '',
                    correoElectronico: data.correo || '',
                    fechaInicio: normalizeDate(data.fechaInicio),
                    tipoOrganizacion: data.tipoOrganizacionId || '',
                    tipoEmpresa: data.tipoEmpresaId || '',
                    tamanoEmpresa: data.tamanoEmpresaId || '',
                    tipoContabilidad: data.tipoContabilidadId || '',
                    nivelFormalizacion: data.nivelFormalizacionId || '',
                    tipoCasos: data.tipoCasoEnProceso || '',
                    participaAsociacion: data.participaGremio || false,
                    beneficiadoCDE: data.beneficiadoCde || false,
                    empleadosCompleto: data.empleadosTiempoCompleto !== null ? data.empleadosTiempoCompleto.toString() : '',
                    empleadosMedio: data.empleadosMedioTiempo !== null ? data.empleadosMedioTiempo.toString() : '',
                    trabajadoresInformales: data.trabajadoresInformales !== null ? data.trabajadoresInformales.toString() : '',
                    negocioEnLinea: data.negocioEnLinea || false,
                    negocioEnCasa: data.negocioEnCasa || false,
                    tipoComercio: data.comercioInternacionalId || '',
                    paisesExporta: data.paisExporta || '',
                    potencialContratoGob: data.contratoGobierno || false,
                    localizadoZonaIndigena: data.zonaIndigena || false,
                    fuenteFinanciamiento: data.fuenteFinanciamientoId || '',
                    subFuente: data.subFuenteFinanciamientoId || '',
                    ingresosAnuales: data.ingresosBrutosAnuales !== null ? data.ingresosBrutosAnuales.toString() : '',
                    fechaIngresos: normalizeDate(data.fechaIngresosBrutos),
                    ingresosExportaciones: data.ingresosExportaciones !== null ? data.ingresosExportaciones.toString() : '',
                    gananciasPerdidas: data.gananciasPerdidasBrutas !== null ? data.gananciasPerdidasBrutas.toString() : '',
                    fechaGanancias: normalizeDate(data.fechaGananciasPerdidasBrutas),
                    adjudicadoFondo: data.fondoConcursable || false,
                    instruccionFondo: data.instrucciones || '',
                    estatusInicial: data.estatusInicial || '',
                    estatusActual: data.estatusActual || '',
                    fechaEstablecimiento: normalizeDate(data.fechaEstablecimiento),
                    impideCrecimiento: data.obstaculos || '',
                    areasNecesita: data.areasADominar || '',
                    dondeDesarrolla: data.lugarDesarrolloEmprendimiento || '',
                    porqueComenzarPotenciar1: data.motivacion || '',
                    porqueComenzarPotenciar2: '', // No hay un segundo campo en el backend
                    descripcionProducto: data.descripcionProductoServicio || '',
                    solicitadoCredito: data.haSolicitadoCredito || false,
                    comoSolicito: data.comoSolicitoCredito || '',
                    porqueNoSolicito: data.porQueNoCredito || '',
                    utilizaMediosPago: data.usaPagoElectronico || false,
                    queMediosPago: data.mediosPago || '',
                    notas: data.notas || '',
                    contactoPrimario: '',
                    nombrePropietario: '',
                    asesorPrincipal: '',
                ...
                // Setear valores seleccionados para los Autocomplete si existen
                if (data.contactoPrimarioId && data.contactoPrimario) {
                    setSelectedContacto({
                        id: data.contactoPrimarioId,
                        label: data.contactoPrimario.nombre + ' ' + data.contactoPrimario.apellido
                    });
                }
                if (data.nombrePropietarioId && data.nombrePropietario) {
                    setSelectedPropietario({
                        id: data.nombrePropietarioId,
                        label: data.nombrePropietario.nombre + ' ' + data.nombrePropietario.apellido
                    });
                }
                if (data.usuarioId && data.usuario) {
                    setSelectedAsesor({
                        id: data.usuario.id,
                        label: data.usuario.nombreCompleto
                    });
                }
                ...
                console.error("Error al cargar cliente/empresa:", error);
                dispatch({
                    type: 'OPEN_SNACKBAR',
                    payload: { open: true, mensaje: 'Error al cargar datos del cliente/empresa.', severity: 'error' },
                });
            }
        };
        cargarClienteEmpresa();
    }, [id, dispatch]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            try {
                const response = await actualizarClienteEmpresa(id, payload);

                if (response.status === 200 || response.status === 204) {
                    dispatch({
                        type: 'OPEN_SNACKBAR',
                        payload: { open: true, mensaje: 'Cliente/Empresa actualizado con éxito.', severity: 'success' },
                    });
